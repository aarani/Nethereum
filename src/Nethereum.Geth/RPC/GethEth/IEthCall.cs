...
namespace Nethereum.Geth.RPC.GethEth
{
    ...
    /// 
    /// Simple example
    /// With a synced Rinkeby node with RPC exposed on localhost (geth --rinkeby --http) we can make a call against the Checkpoint Oracle to retrieve the list of administrators:
    /// 
    ...
    {
        Task<string> SendRequestAsync(TransactionInput transaction, BlockParameter blockParameter, Dictionary<string, StateChange> stateChanges, object id = null, CancellationToken cancellationToken = default(CancellationToken));
