        ...
        public override async Task<JObject> ExecuteAsync(IClient client)
        {
            ...
            var receipt = await
                web3.Eth.DeployContract.SendRequestAndWaitForReceiptAsync(abi, byteCode, senderAddress,
                    new HexBigInteger(900000), CancellationToken.None, multiplier).ConfigureAwait(false);


            var traceTransaction = new TraceGet(client);
            return await traceTransaction.SendRequestAsync(receipt.TransactionHash, new[] { new HexBigInteger(0) }).ConfigureAwait(false);
        }
